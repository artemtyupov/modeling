...
def Poisson_density(k, lmbd):
    chisl = exp(-lmbd) * (lmbd ** k)
    znam = factorial(k)
    return chisl / znam
    
def Poisson_function(k, lmbd):
    k = abs(k)
    sum = 0
    for i in range(k+1):
        sum += (lmbd**i)/factorial(i)
    sum *= exp(-lmbd)
    return sum
...
class Window(QDialog):
    def __init__(self, parent=None):
        ...
        self.tabs.addTab(self.tab2,"Распределение Пуассона")
    ...
    def tab2UI(self):
          ...
          layout.addRow("lambda",self.lmbd)
          layout.addRow("k start",self.k_start)
          layout.addRow("k end",self.k_stop)
    ...
    def get_status(self, type):
        ...
        if tabname == "1":
            ...
            X_d, Y_d = self.getdata_Poisson(Poisson_density)
            if X_d is None:
                return (None, None, None, None)
            X_f, Y_f = self.getdata_Poisson(Poisson_function)
